...
public abstract class EditInstruction<T extends OsmObject> {
	...
	void executeCreate(OsmData osmData, OsmSrcData srcData, int editNumber) 
			throws UnchangedException, Exception {
		...
		osmObject.copyFrom(srcData,osmData);
		osmObject.setDataVersion(editNumber);
	}
	
	
	/** This method executes a delete instruction. */
	void executeDelete(OsmData osmData, OsmSrcData srcData, int editNumber) 
			throws UnchangedException, Exception {
